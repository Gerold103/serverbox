...
namespace sio {
	...
	UnitTestTCPSocketConnect()
	{
		...
		peerSock = TCPSocketConnect(client, host, server);
		mg::net::SocketClose(peerSock);
		TCPSocketWaitClosed(client);
	}
	...
	UnitTestTCPSocketSendRefBuffers()
	{
		...
		mg::net::Buffer::Ptr head = mg::net::BufferRaw::NewShared("123", 3, 0);
		head->myNext = mg::net::BufferCopy::MakeChain("45678", 5);
	...
	UnitTestTCPSocketRecvBuffer()
	{
		...
		while (expectedSize > 0)
		{
			...
			while (rc > 0)
			{
				...
				expectedData = expectedData + toCmp;
				expectedSize -= toCmp;
				rc -= toCmp;
			}
		}
