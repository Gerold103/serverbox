...
public:
	...
	struct ValueChild
		: public ValueBase
	{
		SHARED_PTR_TYPE(ValueChild)
		SHARED_PTR_NEW(ValueChild)
	...
	UnitTestSharedPtrBasic()
	{
		...
		{
			Value v;
			TEST_CHECK(v.myRef.GetValue() == 0);
			TEST_CHECK(!v.myIsDeleted);
			{
				Value::Ptr ptr;
				ptr.Set(&v);
				TEST_CHECK(v.myRef.GetValue() == 1);
				TEST_CHECK(!v.myIsDeleted);
			}
			TEST_CHECK(v.myRef.GetValue() == 0);
			TEST_CHECK(v.myIsDeleted);
	...
	UnitTestSharedPtrAssignCopy()
	{
		...
		// Non-empty = non-empty.
		{
			...
			Value v2;
			{
				Value::Ptr p1(&v1);
				Value::Ptr p2(&v2);
				TEST_CHECK(v1.myRef.GetValue() == 1);
				TEST_CHECK(v2.myRef.GetValue() == 1);
				p1 = p2;
				TEST_CHECK(v2.myRef.GetValue() == 2);
				TEST_CHECK(p1 == &v2 && p2 == &v2);

				TEST_CHECK(v1.myRef.GetValue() == 0);
				TEST_CHECK(v1.myIsDeleted);
			}
			TEST_CHECK(v2.myRef.GetValue() == 0);
	...
	UnitTestSharedPtrInheritance()
	{
		...
		TEST_CHECK(basePtr->myRef.GetValue() == 2);
		basePtr->myRef.Dec();
		basePtr.Clear();
		...
		// Wrap child.
		//
		basePtr = ValueBase::Ptr::Wrap(new ValueChild(1));
		TEST_CHECK(basePtr->myRef.GetValue() == 1);
		basePtr.Clear();
		TEST_CHECK(ValueBase::ourDeleteCount == 1);
		ValueBase::ourDeleteCount = 0;
