...
namespace tcpsocketiface {
	...
	{
		theContext->Reset().CfgSSL().CfgSSLEncrypt(false);
		TestCaseGuard guard("Encrypt after data with data pending");
		...
		mg::box::AtomicBool isEncrypted(false);
		uint64_t id = sock.SubscribeOnEvent([&sock, &isEncrypted, &msg1, &size]() {
			if (isEncrypted.LoadRelaxed())
				return;
			++msg1.myId;
			mg::tst::WriteMessage wmsg;
			msg1.ToStream(wmsg);
			...
			sock.Encrypt();
			isEncrypted.StoreRelaxed(true);
		});
		sock.PostWakeup();
		while (!isEncrypted.LoadRelaxed())
			mg::box::Sleep(TEST_YIELD_PERIOD);
		sock.Unsubscribe(id);
...
namespace tcpsocketiface {
		...
		uint64_t aSize)
	{
		...
		while (stream.GetReadSize() < aSize)
		{
			...
			int64_t rc = aSock.Recv(buf->myWData + buf->myPos, needed, err);
			TEST_CHECK(!err.IsSet());
			if (rc < 0)
			{
				mg::box::Sleep(TEST_YIELD_PERIOD);
