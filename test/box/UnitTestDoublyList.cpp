...
namespace box {
	...
	struct Value
	{
		Value()
			: myValue(-1)
			// Garbage pointer to ensure it is nullified when the element is added to a
			// list.
			, myPrev((Value*)0x123)
			, myNext((Value*)0x123)
		{
	...
	static void
	UnitTestDoublyListInsert()
	{
		TestCaseGuard guard("Insert");
	...
	static void
	UnitTestDoublyListCutLeftExcluding()
	{
		TestCaseGuard guard("CutLeftExcluding");

		// Cut first in 1 size list.
		ValueList list;
		Value v1(1);
		list.Append(&v1);
		list.CutLeftExcluding(&v1);
		CheckLinks(&v1);
		TEST_CHECK(list.GetFirst() == &v1);
		TEST_CHECK(list.GetLast() == &v1);

		// Cut first in 2 size list.
		Value v2(2);
		list.Append(&v2);
		list.CutLeftExcluding(&v1);
		CheckLinks(&v1, &v2);
		...
		// Cut second in 2 size list.
		list.CutLeftExcluding(&v2);
		CheckLinks(&v1);
		CheckLinks(&v2);
		TEST_CHECK(list.GetFirst() == &v2);
		TEST_CHECK(list.GetLast() == &v2);

		// Cut first in 3 size list.
		Value v3(3);
		list.Prepend(&v1);
		list.Append(&v3);
		list.CutLeftExcluding(&v1);
		...
		// Cut third in 3 size list.
		list.Prepend(&v1);
		list.CutLeftExcluding(&v3);
		CheckLinks(&v1, &v2);
		CheckLinks(&v3);
		TEST_CHECK(list.GetFirst() == &v3);
		TEST_CHECK(list.GetLast() == &v3);
	}

	static void
	UnitTestDoublyListCutLeftIncluding()
	{
		TestCaseGuard guard("CutLeftIncluding");

		// Cut first in 1 size list.
		ValueList list;
		Value v1(1);
		list.Append(&v1);
		list.CutLeftIncluding(&v1);
		CheckLinks(&v1);
		TEST_CHECK(list.GetFirst() == nullptr);
		TEST_CHECK(list.GetLast() == nullptr);
	...
	static void
	UnitTestDoublyListCutRightExcluding()
	{
		TestCaseGuard guard("CutRightExcluding");

		// Cut first in 1 size list.
		ValueList list;
		Value v1(1);
		list.Append(&v1);
		list.CutRightExcluding(&v1);
		CheckLinks(&v1);
		TEST_CHECK(list.GetFirst() == &v1);
		TEST_CHECK(list.GetLast() == &v1);
	...
	static void
	UnitTestDoublyListCutRightIncluding()
	{
		TestCaseGuard guard("CutRightIncluding");

		// Cut first in 1 size list.
		ValueList list;
		Value v1(1);
		list.Append(&v1);
		list.CutRightIncluding(&v1);
		CheckLinks(&v1);
		TEST_CHECK(list.GetFirst() == nullptr);
		TEST_CHECK(list.GetLast() == nullptr);
