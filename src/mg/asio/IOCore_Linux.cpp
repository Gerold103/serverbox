...
namespace asio {
	...
	IOCore::PrivPlatformCreate()
	{
		...
		MG_BOX_ASSERT_F(myNativeCore >= 0, "Failed to create epoll, error: %d, %s", errno,
			strerror(errno));

		myEventFd = eventfd(0, EFD_NONBLOCK);
		MG_BOX_ASSERT_F(myEventFd >= 0, "Failed to create eventfd: %d, %s", errno,
			strerror(errno));
		...
		MG_BOX_ASSERT_F(ok, "Failed to register eventfd in epoll: %d, %s", errno,
			strerror(errno));
	}
	...
	IOCore::PrivScheduleDo()
	{
		...
		// element is both in the epoll output and in the front queue output. Firstly, can
		// merge them to avoid double wakeups. Secondly, a task could become CLOSING after
		// epoll output was processed, but before front queue processing. Such CLOSING
		// task would reach a worker thread while the scheduler at the same time could
		// turn it CLOSED and close the socket while it was in use in the worker thread.
		IOTaskForwardList ready;
		// Epoll must be polled before dispatching the front queue. Otherwise it could
		// happen that a task is added to the front after the front was cleared, but
		// before epoll_wait(). The latter would consume latest epollfd value. As a result
		// there would be a non-empty front queue and not signaled epollfd. Not signaled
		// in a sense that epoll_wait() won't return it again. This could lead to a
		// deadlock.
		//
		...
		//                             was empty - update eventfd.
		// (3) Epoll_wait().This consumes
		//   epollfd latest value.
		//
		// If at step (2) something is added to the front queue, after (3) it becomes
		// impossible to wait on eventfd for new front tasks. It would simply block.
		//
		...
		//
		// If poll() returns, it means either timeout, or that the next epoll_wait() will
	...
	IOCore::PrivPlatformSignal()
	{
		...
		MG_DEV_ASSERT_F(ok, "Couldn't write to eventfd: %d, %s", errno, strerror(errno));
	}
		...
		IOTask* aTask)
	{
		...
		MG_DEV_ASSERT(myNativeCore != mg::net::theInvalidSocket);
		epoll_event event;
		memset(&event, 0, sizeof(event));
		...
		MG_DEV_ASSERT_F(ok, "Couldn't add a socket to epoll: %d %s", err,
			strerror(err));
	}
		...
		IOTask* aTask)
	{
		...
		// work, something is seriously wrong.
		MG_DEV_ASSERT_F(ok, "Couldn't remove a socket from epoll: %d %s", err,
			strerror(err));
	}
