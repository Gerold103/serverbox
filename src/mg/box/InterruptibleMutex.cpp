...
namespace {
		...
		const InterruptibleMutexWakeupCallback& aWakeup)
	{
		...
		// flag.
		uint8_t expected = myState.FetchBitOrRelaxed(
		...
		MG_DEV_ASSERT((myState.LoadRelaxed() & INTERRUPTIBLE_MUTEX_TAKEN) != 0);
	}
	...
	InterruptibleMutex::Unlock()
	{
		...
		if (myWaiters.IsEmpty())
		{
			expected = myState.FetchBitAndRelaxed(~INTERRUPTIBLE_MUTEX_HAS_WAITERS);
