...
namespace box {
	...
	Thread::Start()
	{
		MG_BOX_ASSERT(!myWasStarted);
		MG_BOX_ASSERT(!StopRequested());
		myWasStarted = true;
		MG_BOX_ASSERT(!myIsRunning.ExchangeRelease(true));
	...
	{
		myLock.Lock();
		myIsStopRequested.StoreRelease(true);
		myCond.Broadcast();
		myLock.Unlock();
	}
	...
	{
		return myIsStopRequested.LoadAcquire();
	}

	bool
	Thread::IsRunning()
	{
		return myIsRunning.LoadAcquire();
	}
	...
	Thread::PrivTrampoline()
	{
		...
		Run();
		myLock.Lock();
		MG_BOX_ASSERT(myIsRunning.ExchangeRelease(false));
		myCond.Broadcast();
		myLock.Unlock();
	}
