...
// In case of WindowS API non-PC platforms lack some functions
// for getting the error message in an easy way.
#if IS_PLATFORM_WIN
	...
	ErrorWinToString(
		...
		int aFormat)
	{
		// Use English. Other language might not fit into ASCII,
		// and won't be properly printed/logged.
		...
		return mg::box::StringFormat(
			"Failed to get error message for code %d due to %d", aCode, GetLastError());
	}
	...
	ErrorCodeWSAGetLast()
	{
		return ErrorCodeFromWin(WSAGetLastError());
	}
...
#define ERROR_STRING_FROM_FORMAT(Result, FormatArgName) do {								\
		...
		const char* aComment)
	{
		return ErrorRaise(ErrorCodeFromWSA(aValue), aValue,
			ErrorWSAToString(aValue, ERROR_FORMAT_EMPTY).c_str(), aComment);
	}
